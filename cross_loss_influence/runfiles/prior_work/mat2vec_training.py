...
from cross_loss_influence.config import MODEL_SAVE_DIR, PROJECT_NAME
...
if __name__ == "__main__":
    ...
    parser.add_argument("--min_count", default=10, help="Minimum number of occurrences for word.")
    ...
    model_name = args.model_name + f"window-{args.window}_negative-{args.negative}_size-{args.size}"
    all_formula = []
    ...
    sentences, phraser = wordgrams(processed_sentences,
                          ...
                          ip=INCLUDE_PHRASES)
    phraser.save(os.path.join(MODEL_SAVE_DIR, PROJECT_NAME, model_name + "_phraser.pkl"))
    ...
    if not args.notmp:
        class EpochSaver(CallbackAny2Vec):
            ...
            def on_epoch_end(self, m):
                ...
                m.save(output_path)
                self.epoch += 1
        callbacks = [EpochSaver(path_prefix=os.path.join(MODEL_SAVE_DIR, PROJECT_NAME, model_name))]
    else:
        callbacks = []
    my_model = Word2Vec(
        sentences,
        ...
        callbacks=callbacks)
    my_model.save(os.path.join(MODEL_SAVE_DIR, PROJECT_NAME, model_name))
    # print(my_model.wv.most_similar('MS', topn=8))

    # analogy_file = os.path.join("data", "analogies.txt")
    # # Save the accuracies in the tmp folder.
    # compute_epoch_accuracies("tmp", model_name, analogy_file)
